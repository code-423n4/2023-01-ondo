  ...
  Ownable
{
  ...
  constructor(
    ...
    upperBoundAnchorRatio = anchorToleranceMantissa_ > uint(-1) - 100e16
      ? uint(-1)
    ...
    for (uint i = 0; i < configs.length; i++) {
      ...
      if (config.priceSource == PriceSource.REPORTER) {
        ...
        require(
          config.reporter != address(0),
          "reported price must have a reporter"
        );
  ...
  function calculateAnchorPriceFromEthPrice(
    TokenConfig memory config
  ) internal returns (uint anchorPrice) {
  ...
  function convertReportedPrice(
    ...
    uint256 convertedPrice = mul(unsignedPrice, config.reporterMultiplier) /
      config.baseUnit;
