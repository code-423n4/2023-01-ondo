...
interface IOndoPriceOracleV2 is IOndoPriceOracle {
  ...
  function setMaxChainlinkOracleTimeDelay(
    uint _maxChainlinkOracleTimeDelay
  ) external;
  ...
  /**
   * @dev Event for when max chainlink Oracle delay time is set
   *
   * @param oldMaxTimeDelay The old max time delay
   * @param newMaxTimeDelay The new max time delay
   */
  event MaxChainlinkOracleTimeDelaySet(
